...
@requires_py27
class TestThriftGeventWorker:
    def test_connectivity(self, PingService,
                          pingpong_thrift_server_gevent, TSocket, TTransport,
                          TBinaryProtocol):
    ...
    def test_client_timeout(self, PingService,
                            timeout_pingpong_thrift_server_gevent,
    ...
    def test_server_disconnect_connection_when_gracefully_stopping(
            self, PingService, volatile_pingpong_thrift_server_gevent,
            TSocket, TTransport, TBinaryProtocol):
        ...
        # restart
        os.kill(volatile_pingpong_thrift_server_gevent.pid, signal.SIGHUP)
        time.sleep(1)
