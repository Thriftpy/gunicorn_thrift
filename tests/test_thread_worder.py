...
@requires_py27
class TestThriftThreadWorker:
    def test_connectivity(self, PingService,
                          pingpong_thriftpy_server_thread, TSocket, TTransport,
                          TBinaryProtocol):
    ...
    def test_client_timeout(self, PingService,
                            timeout_pingpong_thriftpy_server_thread,
    ...
    def test_server_disconnect_connection_when_gracefully_stopping(
            self, PingService, volatile_pingpong_thriftpy_server_thread,
            TSocket, TTransport, TBinaryProtocol):
        ...
        # restart
        os.kill(volatile_pingpong_thriftpy_server_thread.pid, signal.SIGHUP)
        time.sleep(1)
